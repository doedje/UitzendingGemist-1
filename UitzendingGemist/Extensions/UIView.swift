...
//  UitzendingGemist
//
//  Created by Jeroen Wesbeek on 11/19/15.
//
//
...
extension UIView {
    ...
    func startLoading(backgroundImage: UIImage?) {
        if let overlayView = objc_getAssociatedObject(self, &AssociatedKeys.overlayView) as? UIView {
            ...
            let feedbackLabel = UILabel(frame: CGRectMake(0, feedbackY, bounds.width, 100))
            feedbackLabel.text = "Een ogenblik geduld alstublieft..."
